...
// Search product by barcode, name, or product ID for POS
const searchProductForPos = asyncHandler(async (req, res) => {
  try {
    ...
    // Search by barcode, name, or product ID
    const product = await Product.findOne({
      $or: [
        { barcode: query },
        { "flatVariants.barcode": query },
        { "colorVariants.sizes.barcode": query },
        { name: { $regex: query, $options: "i" } },
        { _id: mongoose.Types.ObjectId.isValid(query) ? query : null },
      ].filter(v => Object.values(v)[0] !== null),
    })
      .populate("brand", "name")
      .populate("category")
      .populate("subCategory")
    ...
    // Detect scanned variant - try exact barcode match first
    ...
    // If no variant matched (searched by name/ID), use single product
    if (!scannedVariant) {
      scannedVariant = {
        productId: product._id,
        productType: "Single",
